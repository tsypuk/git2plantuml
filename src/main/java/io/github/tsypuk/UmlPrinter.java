...
public class UmlPrinter {
        ...
        commitsList.stream().forEach(commit -> {
            if (commit.getParentCommits().size() > 0) {
                commit.getParentCommits().forEach(
                        parentCommit -> drawRelation(commit.getNodeName(), parentCommit));
            }
        });

        //Commit->Tree
        commitsList.stream().forEach(commit -> drawRelation(commit.getNodeName(), commit.getTree().getTreeName()));

        //Tree-Blob
        commitsList.stream().forEach(commit -> {
            Tree tree = commit.getTree();
            tree.getBlobs().stream().forEach(blob -> {
                        drawRelation(tree.getTreeName(), blob.getBlobName());
                    }
            );
        });
    }
    ...
    private void drawRelation(String object1, String object2) {
